...
var s = class {
  constructor(t, e2, i2) {
    this.name = t, this._storage = e2, this.id = h++ + ":", this.size = 0, this.maxSize = 0, this._removeFunc = false, this._hit = 0, this._miss = 0, this._storage.register(this), i2 && (this._storage.registerRemoveFunc(this.id, i2), this._removeFunc = true);
  }
  destroy() {
  ...
  put(t, e2, i2, s2 = 0) {
    this._storage.put(this, t, e2, i2, s2);
  }
  get(t) {
  ...
  updateSize(t, e2, i2) {
    this._storage.updateSize(this, t, e2, i2);
  }
  clear() {
...
var r = class {
  ...
  updateSize(t, e2, s2, r2) {
    ...
    if (h2 && h2.entry === s2) {
      for (this._size -= h2.size, t.size -= h2.size; r2 > this._maxSize; ) {
        const t2 = this._notifyRemove(e2, s2, i.SOME);
        if (!(null != t2 && t2 > 0))
  ...
  pop(t, e2) {
    ...
    if (i2)
      return this._size -= i2.size, t.size -= i2.size, this._db.delete(e2), ++this._hit, i2.entry;
    ++this._miss;
  }
  get(t, e2) {
    e2 = t.id + e2;
    const i2 = this._db.get(e2);
    if (void 0 !== i2)
      return this._db.delete(e2), i2.lives = i2.lifetime, this._db.set(e2, i2), ++this._hit, i2.entry;
    ++this._miss;
  }
  get performanceInfo() {
  ...
  clear(t) {
    ...
    this._db.forEach((t2, s2) => {
      s2.startsWith(e2) && (this._size -= t2.size, this._db.delete(s2), this._notifyRemove(s2, t2.entry, i.ALL));
    }), t.size = 0;
  }
  clearAll() {
    this._db.forEach((t, e2) => this._notifyRemove(e2, t.entry, i.ALL)), this._users.forEach((t) => t.size = 0), this._size = 0, this._db.clear();
  }
  _getHitRate() {
    return this._hit / (this._hit + this._miss);
  }
  _notifyRemove(t, e2, i2) {
    let s2;
  ...
  _checkSizeLimits() {
    if (this._size > this._maxSize) {
      for (const [t, e2] of this._db)
        if (this._purgeItem(t, e2), this._size <= 0.9 * this.maxSize)
          return;
    }
    this._users.forEach((t) => {
      if (t.maxSize > 0 && t.size > t.maxSize) {
        for (const [e2, i2] of this._db)
          if (e2.startsWith(t.id) && (this._purgeItem(e2, i2, t), t.size <= 0.9 * t.maxSize))
            return;
      }
    });
  }
  _purgeItem(t, e2, s2 = this._users.find((e3) => t.startsWith(e3.id))) {
    if (this._db.delete(t), e2.lives <= 1) {
      ...
      const r2 = this._notifyRemove(t, e2.entry, i.SOME);
...
//# sourceMappingURL=chunk-GLVTFP5O.js.map
