...
function c(e2, t) {
  ...
  const a2 = p2.filter((e3) => !s2.includes(e3)), y2 = s2.filter((e3) => !p2.includes(e3)), d = s2.filter((n2) => p2.includes(n2) && l(e2, n2) !== l(t, n2)).concat(a2, y2).sort(), m2 = u(e2);
  ...
  switch (e2.type) {
    ...
    case "collection": {
      ...
      for (const e3 of t.removed)
        if (!y(e3))
          return false;
      for (const e3 of t.changed)
        if (!y(e3))
          return false;
      return true;
    }
    case "partial":
      for (const t in e2.diff) {
        if (!y(e2.diff[t]))
          return false;
      }
      return true;
...
//# sourceMappingURL=chunk-VYG5A4VI.js.map
