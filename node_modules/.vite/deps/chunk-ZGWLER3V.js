...
var D = 3;
var b = r(D * parseFloat(Number(65e-8).toFixed(6)), D * parseFloat(Number(1881e-9).toFixed(6)), D * parseFloat(Number(85e-9).toFixed(6)));
var P = 3996e-9;
var w = r(parseFloat(Number(y[0] + b[0]).toFixed(6)), parseFloat(Number(y[1] + b[1]).toFixed(6)), parseFloat(Number(y[2] + b[2]).toFixed(6)));
function S(t2) {
  const a2 = new i2();
  a2.attributes.add(O.POSITION, "vec2"), a2.include(o2, { textureCoordinateType: d.Default }), a2.varyings.add("worldRay", "vec3"), a2.varyings.add("eyeDir", "vec3");
  const { vertex: D2, fragment: b2 } = a2;
  return D2.uniforms.add(new e4("inverseProjectionMatrix", (e9, t3) => t3.camera.inverseProjectionMatrix), new e4("inverseViewMatrix", (t3, a3) => e(F, a3.camera.viewMatrix))), D2.code.add(o`void main(void) {
vec3 posViewNear = (inverseProjectionMatrix * vec4(position, -1, 1)).xyz;
eyeDir = posViewNear;
worldRay = (inverseViewMatrix * vec4(posViewNear, 0)).xyz;
forwardTextureCoordinates();
gl_Position = vec4(position, 1, 1);
}`), b2.uniforms.add(new e3("backgroundColor", (e9) => e9.backgroundColor), new e5("radii", (e9) => e9.radii), new e3("cameraPosition", (e9, t3) => t3.camera.eye), new e6("heightParameters", (e9) => e9.heightParameters), new o3("innerFadeDistance", (e9) => e9.innerFadeDistance), new o3("altitudeFade", (e9) => e9.altitudeFade), new s("depthTexture", (e9) => e9.depthTexture), new o3("hazeStrength", (e9) => e9.hazeStrength)), b2.constants.add("betaRayleigh", "vec3", y), b2.constants.add("betaCombined", "vec3", w), b2.constants.add("betaMie", "float", P), b2.constants.add("scaleHeight", "float", r2 * n), t(b2), a2.include(e7), t2.haze && (b2.include(a), b2.uniforms.add(new e5("nearFar", (e9, t3) => t3.camera.nearFar))), b2.code.add(o`vec2 sphereIntersect(vec3 start, vec3 dir, float radius, bool planet) {
float a = dot(dir, dir);
...
}`), b2.code.add(o`float chapmanApproximation(float X, float h, float cosZenith) {
float c = sqrt(X + h);
float cExpH = c * exp(-h);
if (cosZenith >= 0.0) {
...
}`), b2.code.add(o`float getOpticalDepth(vec3 position, vec3 dir, float h) {
...
//# sourceMappingURL=chunk-ZGWLER3V.js.map
